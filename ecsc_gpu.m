function res = ecsc_gpu(D0, S0, plan, isTrainingDictionary,folder,original)
    ...
    gX      = gpuArray.zeros(blobSize, 'single');
    gY      = gpuArray.zeros(blobSize, 'single');
    gYprv   = gY;
    gXf     = gpuArray.zeros(blobSize, 'single');
    gYf     = gpuArray.zeros(blobSize, 'single');

    % gS      = gS0; 
    %gSf     = gpuArray.zeros(dataSize);

    gD      = gpuArray.zeros(blobSize, 'single');
    gG      = gpuArray.zeros(blobSize, 'single');
    gGprv   = gpuArray.zeros(blobSize, 'single');

    gD      = gpuArray.zeros(blobSize, 'single');
    gG      = Pzp(gD); % Zero pad the dictionary
    gGprv   = gG;

    gDf     = gpuArray.zeros(blobSize, 'single');
    gGf     = gpuArray.zeros(blobSize, 'single');

    gU      = gpuArray.zeros(blobSize, 'single');
    gH      = gpuArray.zeros(blobSize, 'single');

    gGf     = gpuArray.zeros(blobSize, 'single');
    gGf     = fft3(gG);
    
    % Temporary buffers
    gGSf    = gpuArray.zeros(blobSize, 'single');
    gYSf    = gpuArray.zeros(blobSize, 'single');
